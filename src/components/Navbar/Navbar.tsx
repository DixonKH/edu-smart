...
const Navbar = (props: Props) => {
  return (
    <div className="">
      <div className="flex justify-between items-center bg-green1">
      ...
      <div className="flex justify-between items-center w-auto p-2 bg-background">
        ...
        <div className="flex justify-between items-center gap-5">
          <ul className="flex items-center justify-between gap-5 text-xl">
            {NavbarItems.map((item, index) => (
              <li
                className=" font-nunito px-3 "
                key={index}
              >
                <Link className="hover:border-green1 hover:border-b-2 hover:text-green1" to={item.link}>{item.name}</Link>
              </li>
            ))}
          </ul>
          <Link to="mypage" className="flex items-center justify-between gap-2 bg-yellow py-1 px-3 rounded-xl hover:border-green1 hover:border-b-2 hover:text-green1">
            <IoPerson className="text-xl text-bgGreen" />
            <span  className="text-black text-xl font-semibold">My Page</span>
          </Link>
        </div>
      </div>
    </div>
